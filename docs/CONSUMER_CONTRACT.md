...
| `radar prd [<repo>] [--gap <ID>] [--floor N] [--project NAME] [--with-fixtures]` | a build-loop `prd.json` whose FIRST story reproduces the gap as a failing test. `--with-fixtures` is OPT-IN and APPENDS the reproduction sample's file CONTENTS to the `reproductionSample` object the default arm already carries -- one `{relative path: content}` object per side, name-sorted, bytes verbatim -- so a consumer that cannot read this register's own record files receives the sample rather than a pointer to it. Without the flag the document is byte-identical to what it has always been, including the US-001 criterion, which names the record glob on the default arm and the inlined key path on the flagged one: the instruction is derived from the payload, so a loop is never sent to another repo for bytes it was handed. `radar scan --prd` deliberately has no such flag; it emits the default arm, and the two surfaces stay byte-comparable |
| `radar scan <target> [--gaps R] [--gap <ID>] [--json] [--floor N] [--prd] [--exit-code]` | applies every register check to a concrete repo: PRESENT / ABSENT / NOT_APPLICABLE / MANUAL / UNKNOWN per gap, with file:line locators; `--prd` emits a prd.json for the worst PRESENT finding whose confidence clears the register floor, names each skipped below-floor finding on stderr, and exits 2 if none clears; `--exit-code` leaves the document byte-identical whenever it verdicts and moves the ANSWER into the exit status instead -- 1 when this target has at least one PRESENT finding that clears the floor, 0 when it has none, 2 when the scan applied zero records and so verdicted nothing. The two flags are mutually exclusive: both are floor-gated verdict surfaces, and they answer the same question with opposite codes. `--floor N` sets the floor all three floor-reading surfaces apply -- `--prd`'s selection and the floor its `Note:` and refusal lines name, `--exit-code`'s verdict, and the `confidence_floor` / `below_floor` fields `--json` publishes -- defaulting to the register floor, so every documented invocation without it is byte-identical to what it was. It gates VERDICTS only: the markdown document and the `findings` array are the same records in the same order at every floor, because a below-floor record is DISPLAYED and never dropped. `--gap` narrows the record DOMAIN to the single named record BEFORE the checks run, so `Register records applied:` reads `1` and the one finding is byte-for-byte the finding the unnarrowed scan reports for that id -- the flag changes what was ASKED, never a verdict, a confidence or the floor. Note the one-character distinction from `--gaps`, which is the register LOCATION: `--gap` is a record id, an unknown one is exit 2, and it is never a shorthand for the other. Over that one-record domain `--exit-code` refuses (exit 2) where an unnarrowed scan would answer `0`, whenever the selected record reached no automated verdict -- `MANUAL`, `UNKNOWN`, or no check at all -- because a domain of one applies 1 record even when nothing was decided, and `0` is published as an ANSWER |
| `radar diff <old> <new> [--json] [--exit-code]` | what changed between two register states: records added, records removed, and per-record changes across a CLOSED set of nine fields (`status`, `layer`, `gap_type`, `severity`, `frequency`, `tractability`, `priority`, `confidence`, citation count). Free prose is never compared, so a rewording is not a change; both domain sizes are stated, so an emptied, moved or one-level-too-high side cannot read as "everything was added"; both paths are REQUIRED, so neither side can silently default to the caller's working directory; `priority` and `confidence` are reported as two separate lines and never blended. `--json` emits that same comparison as a stable object instead of the report, so the non-regression half of the gate rule below is ASSERTABLE rather than scraped: `counts.old` and `counts.new`, `added` and `removed` as `gap_id`/`title` entries in the order the report lists them, and per changed record only the fields that DIFFER, in that same closed order, carrying both sides exactly as they were compared. `priority` and `confidence` stay two distinct `field` entries there too, and no object at any depth carries a blended `score` key. `--exit-code` moves the NON-REGRESSION half of the gate rule below into the exit status, so a consumer no longer has to invent its own regression predicate over that object: **`1`** when the new state LOST ground -- a record the old side carried is gone, or a record's `status` moved OUT of `taxonomy.terminal_statuses()`, meaning a gap that was called done is open again -- **`0`** when neither happened, and **`2`** when the OLD side holds zero records, because an empty baseline makes every record read as ADDED and nothing as removed, which is a clean gate no pair earned. A record ADDED and a FALLING `confidence` or `priority` are deliberately NOT regressions: the register growing, and its evidence ladder being honestly re-scored after a source was voided, must never red a build. OPT-IN, and the document is byte-identical to the same invocation without the flag on BOTH surfaces -- stdout carries the report or the object exactly as before, stderr stays empty, and only the process's exit status changes |
...
|---|---|---|
| `0` | the verb produced its document. Usage text reaches stdout only when a caller asks for it with `--help`; no verb answers `0` with a usage block in place of a document, and bare `radar` (no verb at all) refuses with `2` rather than printing help, so a `0` with usage on stdout can only ever be the `--help` the caller requested | read stdout |
| `1` | OPT-IN, and returned ONLY by a verb asked a verdict question: today `scan --exit-code` and `diff --exit-code`, the two halves of the one gate rule published under `## What a consumer must never do`. From `scan`: the tool answered, and this target exhibits at least one PRESENT gap whose evidence clears the confidence floor. From `diff`: the new register state lost ground -- a record the old side carried is gone, or a gap that had reached a terminal status is open again. Neither is a failure: the tool answered, and the answer is bad news about the thing under test. On both, stdout carries the same document as the same invocation without the flag, and stderr is empty | fail the gate, then read stdout: for `scan`, the `## Actionable now` section (or `--json`'s `findings`) for which gaps and where; for `diff`, the `## Removed` section and the `status` lines under `## Changed` (or `--json`'s `removed` and `changed`) for what was lost |
| `2` | the tool refuses and says why: stderr's LAST non-empty line begins `Error: `, and stdout stays empty rather than half a document. `scan --gap <ID>` refuses here twice: for an id no register record carries (`Error: no such gap: <ID>`, the same sentence `prd --gap` produces for the same input), and -- with `--exit-code` -- when the ONE selected record reached no automated verdict, because over a one-record domain the zero-records refusal cannot fire and `0` would publish a clean gate the scan never earned. Deliberately not promised as a SINGLE line: a structural refusal -- a missing verb (bare `radar` with no verb at all), a missing positional, an unknown verb, an unparseable flag value, an unrecognized option -- prints the `usage:` block ABOVE that line, because usage is the only place a caller learns the arity it got wrong. A strict prefix of a long flag (`--flo` for `--floor`, `--exit` for `--exit-code`) is a structural refusal of that last kind (`Error: unrecognized arguments: ...`, exit 2) on every verb; only the exact spellings in the table above are published, so a flag added later can never silently rebind a prefix a script already spells. The prefixed line is always the last one, and argparse's own `<prog>: error: ...` spelling never reaches stderr, so one deterministic line answers every exit-2 door | read stderr's LAST non-empty line -- it names the register, the path, the record or the argument at fault |
...
now arrived: it is the VERDICT code, which is why the row above describes a gate
result rather than a failure. It is shared by the two surfaces that answer the
two halves of the gate rule this document binds a consumer to -- `scan
--exit-code` for honesty (roadmap row 25) and `diff --exit-code` for
non-regression (row 131) -- and deliberately not split into a code per verb, so a
consumer running both writes one comparison against 1 rather than a table.

One property is shared and worth stating plainly, because a gate is built on it:
the code is OPT-IN on both, so no invocation that works today can start returning
it -- `scan` and `diff` without the flag still exit 0 whatever they find.

Each half then carries its own guard against reddening a build over honest work,
and neither guard is the other's. `scan --exit-code` is FLOOR-GATED, so a record
whose only evidence is model output can never turn a build red: below-floor
PRESENT findings still appear in the document, and they report 0. That floor rule
has one implementation, shared with `--prd`, so the two cannot disagree about one
target. `diff --exit-code` instead counts exactly two shapes as lost ground -- a
record REMOVED, and a `status` leaving `taxonomy.terminal_statuses()` -- so the
register growing a record, or re-scoring one DOWN after a source was voided,
reports 0. A falling `confidence` is the evidence ladder being honest, and a tool
that failed a build for it would teach a loop to stop re-scoring.
